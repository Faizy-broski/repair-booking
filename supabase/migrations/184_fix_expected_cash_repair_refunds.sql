-- Migration 184: Subtract the cash portion of a repair refund from Expected
-- Cash.
--
-- When a repair is marked "Refunded" directly in the Repairs module
-- (RepairController.updateStatus), repairs.refund_amount is set to the
-- deposit_paid, but neither register_session_expected (live tile) nor
-- close_register_session (Z-report) ever subtracted this from v_expected —
-- only cash refunds recorded against a `sales` row (v_cash_refunds) were.
-- So any repair with a cash-paid deposit that gets refunded leaves cash
-- physically in the customer's hand while Expected Cash stays unchanged,
-- producing a real "counted cash is short" variance equal to the refunded
-- amount.
--
-- Fix: for refunds surfaced via the existing refunded_elsewhere CTE (a
-- repair marked Refunded with no matching `sales` row — i.e. not already
-- captured by v_cash_refunds), look up how much of that repair's payments
-- (repair_payments) were actually cash, capped at what was refunded, and
-- subtract it from v_expected. Kept in lockstep across both functions per
-- the established pattern (see 172/173) — this codebase has repeatedly
-- regressed by fixing only one of the two.
