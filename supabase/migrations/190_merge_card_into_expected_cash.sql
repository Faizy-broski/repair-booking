-- Migration 190: Merge card payments into "Expected Cash" — EXPLICIT
-- BUSINESS DECISION, not a bug fix.
--
-- Every prior migration (172 through 189) treated "Expected Cash" as
-- physical cash that should be in the till drawer, deliberately excluding
-- card payments (card money settles to a bank account, never the drawer).
-- The business owner explicitly requested changing this: card payments
-- should now be folded into the same "Expected Cash" figure shown live
-- throughout the day, while the Cash/Card breakdown remains visible
-- separately in the Z-report (migration 189).
--
-- Because staff can only ever physically COUNT cash (bills/coins) at
-- closing, not card money, close_register_session() now also takes a new
-- p_closing_card_total parameter — staff type in the day's card total from
-- their card machine's own report, and the app adds that to the counted
-- cash before comparing against the new merged Expected Cash, keeping the
-- Over/Short check meaningful instead of always showing a fake shortfall
-- equal to the day's card takings.
--
-- Every card-side addition here mirrors the existing cash-side one
-- one-for-one: v_card_refunds mirrors v_cash_refunds, v_credit_repayments_card
-- mirrors v_credit_repayments_cash, v_repair_card_deposits mirrors
-- v_repair_cash_deposits (both deliberately excluding pos_paid, since
-- pos_paid's card portion is already fully captured via v_card_sales's raw
-- s.total — same reasoning as the cash version, see migration 184/185's
-- comments), and repair_card_refunded mirrors repair_cash_refunded.
-- v_card_sales itself needs no new logic — it already holds the full raw
-- total (including split-tender's card leg, folded in by migration 189).

ALTER TABLE register_sessions ADD COLUMN IF NOT EXISTS closing_card_total NUMERIC DEFAULT 0;
...
BEGIN
  ...
  v_expected := v_session.opening_float
    + v_cash_sales + v_split_tender_cash + v_repair_cash_deposits + v_credit_repayments_cash
