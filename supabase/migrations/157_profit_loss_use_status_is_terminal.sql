-- Replaces get_profit_loss's guessed "is this repair finished?" checks
-- (inline LOWER(status) IN (...) OR LIKE '%collect%' etc., duplicated 3x)
-- with a lookup against the new repair_custom_statuses.is_terminal flag
-- (see migration 156) — the explicit, business-owned source of truth.
--
-- Every other key/value/formula this function returns is UNCHANGED from
-- migration 155 (revenue, total_revenue, cogs, sales_cogs, repair_parts_cogs,
-- expenses, salaries, total_costs, gross_profit, net_profit,
-- repair_revenue_pos/repair_revenue_direct split, repair_revenue_booked) —
-- only the terminal-status *test* itself changes, from guessing to lookup.
