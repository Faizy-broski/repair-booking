-- Switches get_profit_loss's "completed this period" repair figures from the
-- updated_at proxy to the real completed_at timestamp added in migration 182.
--
-- Everything else is UNCHANGED from migration 180 (discount_amount fallback,
-- repair_custom_statuses terminal lookup, POS-override join, COGS, lab fees,
-- expenses, salaries, v_cash_net, all returned JSON keys, and the
-- booking-window v_repairs_booked block which uses created_at and is
-- unchanged) — only the three `r.updated_at::date BETWEEN ...` filters
-- (repair revenue, repair parts COGS, repair lab fees) become
-- `r.completed_at::date BETWEEN ...`.
...
BEGIN
  ...
  -- sum is v_repairs, unchanged from before. Now gated on completed_at
  ...
  -- part of any total/gross/net profit math below. Unchanged — still keyed
  -- off created_at, not completion date.
  SELECT COALESCE(SUM(
    CASE
