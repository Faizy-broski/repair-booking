-- Migration 189: Split-tender sales were miscategorized entirely as "Other
-- Sales" instead of being broken into their Cash/Card legs.
--
-- The main per-tender aggregation in both register_session_expected() and
-- close_register_session() buckets a sale by its single payment_method
-- column (cash/card/store_credit/loyalty_points/on_account/else-"other").
-- A split-tender sale's payment_method is literally 'split', which matches
-- none of those named buckets, so its ENTIRE total (both legs) fell into
-- the NOT IN (...) catch-all -> v_other_sales. A separate CTE (added in
-- migration 173) already recovered just the CASH leg from payment_splits
-- into v_split_tender_cash -- but that value only ever fed the Expected
-- Cash formula, never the Cash Sales tile, and there was no card-leg
-- equivalent at all. Net effect: a £100 split sale (£25 cash / £75 card)
-- showed up as £100 "Other Sales", not £25 Cash Sales + £75 Card Sales, on
-- the register bar, close-register modal, and Z-report.
--
-- Fix: extend the existing payment_splits CTE to also recover the card leg
-- (and any other non-cash/non-card leg, e.g. a custom channel used in a
-- split) into their own totals, fold those into v_card_sales/v_other_sales
-- after the main aggregation (same "adjust after computing" pattern already
-- used for v_on_account_sales += v_credit_repayments_total), and add 'split'
-- to v_other_sales's own exclusion list so a split sale's full total isn't
-- ALSO double-counted there. The CTE's WHERE clause intentionally stays
-- `payment_method <> 'cash'` (not narrowed to `= 'split'`) because it also
-- has to catch on_account sales with a cash-tendered deposit (see the
-- existing Group A4 regression test) -- extending that same broad condition
-- to the card/other legs keeps both cases handled by one consistent rule.
--
-- Both register_session_expected() and close_register_session() are
-- redefined together per the established "keep both in sync" convention
-- (see 172/173/184/185); the rest of the formula is byte-identical to 185.
