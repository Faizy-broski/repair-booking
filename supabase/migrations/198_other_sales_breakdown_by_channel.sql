-- Migration 198: Break out "Other Sales" by individual payment channel.
--
-- Riseteck is the only business with custom split-payment channels beyond
-- Cash/Card (eBay, Deliveroo, Website — gated via
-- business_module_access.settings_override.custom_payment_channels, see
-- migration 106). Both register functions have always lumped every
-- non-cash/non-card tender (direct or a split leg) into one generic
-- v_other_sales figure, by explicit design (see migration 189's header) —
-- correct for Expected Cash (none of these are physical/card-machine
-- money, so none of them should be part of the drawer reconciliation
-- either way), but it means Riseteck can't see eBay vs Deliveroo vs
-- Website individually anywhere except the Reports > Payments page (an
-- accidental side effect of how that page's query happens to group split
-- legs — see Part 12 of the plan).
--
-- Fix: add a new v_other_sales_breakdown JSONB field — a {method: amount}
-- object built from the exact same two sources v_other_sales itself
-- already sums (direct non-split sales paid via a non-cash/card/
-- store_credit/loyalty/on_account method, plus non-cash/non-card split
-- legs), just grouped by method instead of collapsed into one scalar. Its
-- values always sum to exactly v_other_sales — same money, finer detail.
--
-- Deliberately generic (not hardcoded to 'ebay'/'deliveroo'/'website') so
-- it automatically covers any future custom channel for Riseteck or any
-- other business granted custom_payment_channels later, with no further
-- migration needed. For every business without custom channels configured,
-- this object is simply '{}' and nothing else changes.
--
-- v_other_sales, v_expected, and every other existing field/formula are
-- UNCHANGED — this migration only adds the one new field, plus persists it
-- (alongside a new register_sessions.other_sales_breakdown column) so
-- CLOSED/historical sessions have it too, not just the live preview —
-- same reasoning as migration 197's repair_cash_deposits/
-- repair_card_deposits addition.

ALTER TABLE register_sessions ADD COLUMN IF NOT EXISTS other_sales_breakdown JSONB DEFAULT '{}'::jsonb;
