-- Migration 130: A payment collected today against an OLD on-account sale
-- (via record_credit_payment / the "Record Payment" button on the credits
-- page) was invisible to today's live stats and Expected Cash. That RPC only
-- updates sales.amount_paid in place (sales.created_at stays frozen at the
-- original sale date) and appends a row to sale_payments (migration 120) —
-- but every stats aggregate filters by sales.created_at >= session.opened_at,
-- so a same-day repayment on a prior-session sale never shows up anywhere,
-- even though real cash may have just been collected.
--
-- Fix: fold same-day repayments on prior-session sales into the Credit Sales
-- tile (v_on_account_sales) and, when paid in cash, into Expected Cash.
-- Deliberately NOT added to Net Sales / v_total_sales — the sale's full
-- value was already recognized as revenue in the session it was created in;
-- counting it again here would double-count revenue. This only surfaces the
-- cash-collection event, not a second sale.
--
-- Repayments on a sale created THIS SAME session are already reflected
-- correctly today (v_on_account_sales already reads the live amount_paid for
-- sales created in this session), so this only needs to cover sales created
-- in a PRIOR session — hence the `s.created_at < v_session.opened_at` guard,
-- which prevents double-counting those same-session repayments twice.
