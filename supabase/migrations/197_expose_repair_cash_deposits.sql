-- Migration 197: Expose repair_cash_deposits in the RPC's returned JSON.
--
-- Both register functions have always computed v_repair_cash_deposits
-- internally (it feeds the v_expected formula directly), but only ever
-- returned its sibling, repair_card_deposits (added by migration 190) --
-- repair_cash_deposits itself was never included in the RETURN payload.
-- This asymmetry blocks the frontend from building a correct combined
-- "Total Cash Sales" figure the same way it already can for card
-- (card_sales + repair_card_deposits) -- see Part 11 of the plan, written
-- after a client found the Cash/Card Sales breakdown confusing: Product
-- and Repair tenders are shown in two separate, identically-named sections
-- with no combined total anywhere to check against a card machine report
-- or a cash count.
--
-- Purely additive: no formula, join, or existing field changes -- every
-- line is unchanged from migration 194 except the one new key added to
-- each RETURN jsonb_build_object(...). repair_cash_deposits already
-- excludes POS-cart-paid repairs (same as repair_card_deposits), so
-- `cash_sales + repair_cash_deposits` on the frontend will be guaranteed
-- to reconcile with Expected Cash's own cash-side sum, with no double
-- counting, by construction -- mirroring the existing card_sales +
-- repair_card_deposits pattern exactly.
--
-- Also adds two new columns to register_sessions (repair_cash_deposits,
-- repair_card_deposits) and persists them in close_register_session()'s
-- UPDATE -- without this, a CLOSED session's Total Cash/Card Sales tiles
-- (Excel export, historical Z-Report view) would have no source for these
-- two numbers at all, since those views read the table directly rather
-- than calling the RPC live. Both new columns default to 0 and are purely
-- additive; no existing column, index, or constraint changes.

ALTER TABLE register_sessions ADD COLUMN IF NOT EXISTS repair_cash_deposits NUMERIC DEFAULT 0;
ALTER TABLE register_sessions ADD COLUMN IF NOT EXISTS repair_card_deposits NUMERIC DEFAULT 0;
