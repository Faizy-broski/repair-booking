-- Migration 125: "Credit Sales" should reflect the deposit actually
-- collected today, not the full order value.
--
-- Migration 123 summed `total` (the full sale amount) for on_account sales
-- into the "Credit Sales" bucket. But `total_sales`/"Product Sales" already
-- sums `total` across EVERY payment method, including on_account — so an
-- on-account sale's full value was being counted once inside Product Sales
-- and a second time (nearly identically) inside Credit Sales, making the
-- two POS live-stats tiles look like duplicates of each other.
--
-- Fix: Credit Sales now sums `amount_paid` (the deposit actually collected
-- at checkout — see the "Deposit paid now" field in cart-panel.tsx's Credit
-- Payment modal) instead of `total`. The un-deposited remainder is a
-- receivable (tracked in Customer/Employee credit), not cash collected
-- today, so it no longer inflates this figure.
