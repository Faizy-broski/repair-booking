-- Fixes get_profit_loss() overstating repair revenue for discounted repairs.
--
-- When a completed repair has no actual_cost set, revenue fell back to the
-- bare estimated_cost without subtracting discount_amount. Every other
-- repair-revenue calculator in the app already subtracts it in this
-- situation — see computeRepairRevenue() in repair-financials.service.ts:
--   fullCost = actual_cost ?? Math.max(0, estimated_cost - discount_amount)
-- (actual_cost, when set, is the final agreed amount and already accounts
-- for any discount; estimated_cost is the fixed gross total, so the
-- discount is only subtracted when falling back to it.) get_profit_loss
-- disagreed with its own reference figures for the Repairs dashboard's
-- "completed (P&L basis)" numbers as a result.
--
-- Every other field/formula this function returns is UNCHANGED from
-- migration 178 (repair_custom_statuses terminal lookup, POS-override join,
-- COGS, lab fees, expenses, salaries, v_cash_net purpose exclusions, all
-- returned JSON keys) — only the two `COALESCE(r.actual_cost,
-- r.estimated_cost, 0)` fallbacks (in the v_repairs/_pos/_direct query and
-- the v_repairs_booked terminal-status branch) now subtract discount_amount.
...
BEGIN
  ...
  -- sum is v_repairs, unchanged from before. Fallback to estimated_cost now
  -- subtracts discount_amount (see header comment above).
  SELECT
  ...
  -- part of any total/gross/net profit math below. Terminal-status branch
  -- now subtracts discount_amount too, same fix as above.
  SELECT COALESCE(SUM(
    CASE
