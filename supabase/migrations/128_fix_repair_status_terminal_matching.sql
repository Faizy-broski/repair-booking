-- Migration 128: register_session_expected / close_register_session only
-- counted a repair's remaining balance into Repair Sales / Net Sales when
-- its status was the literal string "collected" (or "refunded"). Businesses
-- name their terminal repair status freely (repair_custom_statuses.name is
-- free text, see migration 064) — e.g. "Picked Up", "Completed", "Repaired"
-- — so any shop not using the exact word "Collected" silently never got
-- that revenue counted. RepairService.getRevenueStats and GET
-- /api/repairs/stats already solved this with fuzzy terminal-status
-- matching; these two SQL functions are the only place still doing a
-- literal match. Fix: reuse that same matching logic here via two small
-- helper functions.

CREATE OR REPLACE FUNCTION is_repair_status_collected(p_status TEXT)
RETURNS BOOLEAN LANGUAGE sql IMMUTABLE AS $$
  SELECT p_status IS NOT NULL
    AND lower(trim(p_status)) <> 'refunded'
    AND (
      lower(trim(p_status)) = ANY (ARRAY['repaired','collected','unrepairable','completed','done','fixed','closed','picked_up','handover'])
      OR lower(trim(p_status)) LIKE ANY (ARRAY['%complet%','%done%','%fixed%','%pick%','%closed%','%resolv%','%finish%','%collect%','%handover%'])
    )
$$;

CREATE OR REPLACE FUNCTION is_repair_status_refunded(p_status TEXT)
RETURNS BOOLEAN LANGUAGE sql IMMUTABLE AS $$
  SELECT p_status IS NOT NULL AND lower(trim(p_status)) = 'refunded'
$$;
