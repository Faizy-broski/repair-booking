-- Migration 196: Trade-in payouts have the exact same double-counting bug
-- that migration 195 just fixed for buybacks — missed there because that
-- fix was scoped to the specific "buyback" report, but the same root cause
-- applies to trade-ins too.
--
-- TradeInService.create() (src/backend/services/advanced-inventory.service.ts)
-- sets the traded-in item's cost basis to the trade-in value — either an
-- inventory_cost_layers row (source_type='trade_in', unit_cost=trade_in_value)
-- for FIFO/LIFO-valuation products, or update_average_cost(..., trade_in_value)
-- for weighted-average ones. Either way, when that item is later resold,
-- consume_and_freeze_cost() (migration 136) consumes from that cost basis and
-- freezes it onto sale_items.unit_cost — so COGS at resale includes the full
-- trade-in value, same mechanism as a buyback product's cost_price. But
-- purpose='trade_in' was never added to v_cash_net's exclusion list, so it
-- was (and until this migration is applied, still is) also subtracted
-- immediately as a loss on the day of the trade-in — double-counted exactly
-- like buyback was before migration 195.
--
-- Fix: add purpose='trade_in' to the same exclusion branch as 'buyback'.
-- Every other line of this function is unchanged from migration 195.
...
BEGIN
  ...
  -- tagged purpose = 'expense' (already counted via v_expenses above),
  -- 'plain' (UI explicitly promises these have no report effect), 'buyback'
  -- or 'trade_in' (both recognized as COGS at resale instead — see this
  -- migration and migration 195's header), and cash-ins tagged
  -- purpose = 'gift_card_sale' (deferred revenue, not revenue — migration 187).
  SELECT COALESCE(SUM(CASE
      ...
      WHEN type = 'cash_out' AND purpose = 'trade_in' THEN 0
      ELSE -amount
    END), 0)
  INTO v_cash_net
  FROM cash_movements
