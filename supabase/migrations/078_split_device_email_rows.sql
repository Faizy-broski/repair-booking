-- Migration 078: Split device into separate Brand / Model rows in all email templates.
-- Replaces the combined "{{device_type}} {{device_brand}} {{device_model}}" single cell
-- with two separate rows: Brand and Model.
...
<p style="margin:0;font-size:14px;color:#6b7280;">We''ll keep you updated on the progress. Thank you for choosing <strong>{{store_name}}</strong>.</p>',
     'Hi {{customer_name}}, your repair ticket {{ticket_number}} for {{device_brand}} {{device_model}} has been created. Issue: {{issue}}. We''ll keep you updated. — {{store_name}}'),
...
-- ── 2. Update all existing templates that still use the combined device cell ──
-- Uses REPLACE to surgically swap the old single Device row for two rows (Brand + Model).
-- Alternating background colours are preserved correctly across the new rows.
...
SET
  ...
  updated_at = NOW()
WHERE email_body LIKE '%{{device_type}} {{device_brand}} {{device_model}}%';
