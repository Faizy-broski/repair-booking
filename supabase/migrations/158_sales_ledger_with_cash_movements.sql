-- 158 — unified Sales-page ledger: sales/refunds/exchanges + register cash in/out
-- Additive only: the existing /api/pos/sales path (PosService.getSales) and the
-- `sales` table are untouched. This adds a read-only function the Sales page
-- uses to show cash_movements rows inline with sales, correctly paginated
-- and sorted across both sources.
