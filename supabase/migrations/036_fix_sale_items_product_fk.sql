-- Migration 036: Fix process_sale to null out product_id for service items
-- Prevents FK violation when repair virtual products or misc items are sold.
-- Service items (is_service=true) never reference the products table so their
-- product_id must always be stored as NULL.
