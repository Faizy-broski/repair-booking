...
export const InvoiceSettingsService = {
  ...
  async get(businessId: string, branchId?: string | null): Promise<InvoiceSettings> {
    ...
    return {
      ...
      footer_line_1_scope: row.footer_line_1_scope ?? 'both',
      footer_line_2_scope: row.footer_line_2_scope ?? 'both',
      ...
      policy_text_pos: row.policy_text_pos ?? null,
      policy_text_repair: row.policy_text_repair ?? null,
