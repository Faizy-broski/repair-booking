...
export function CustomerSearch({ value, onChange }: CustomerSearchProps) {
    ...
    return (
      <div className="flex items-center gap-2 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-on-surface-variant truncate">{value.phone ?? value.email ?? 'No contact info'}</p>
        </div>
        <button onClick={clear} className="shrink-0 rounded p-0.5 text-outline hover:text-on-surface-variant">
          <X className="h-4 w-4" />
        </button>
      </div>
    )
  ...
  return (
    <div ref={containerRef} className="relative">
      {/* Search input */}
      <div className="flex items-center gap-2 rounded-lg border border-outline bg-surface px-3 py-2 focus-within:border-blue-500">
      ...
      {showDropdown && query.trim() && (
        <div className="absolute z-50 mt-1 w-full rounded-lg border border-outline-variant bg-surface shadow-lg">
          {results.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-on-surface truncate">
                        ...
                        {c.business_name && (
                          <span className="ml-1.5 inline-flex items-center rounded bg-surface-container px-1.5 py-0.5 text-[10px] font-medium text-on-surface-variant">
                            {c.business_name}
                          </span>
                        )}
                      </p>
            ...
            !isSearching && (
              <div className="px-3 py-2 text-sm text-on-surface-variant">No customers found</div>
            )
          )}
      ...
      {showQuickAdd && (
        <div className="mt-2 rounded-lg border border-outline-variant bg-surface-container-low p-3 space-y-2">
          <p className="text-xs font-semibold text-on-surface-variant uppercase tracking-wide">New Customer</p>
