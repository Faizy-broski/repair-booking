...
export default function RepairCustomerDetailPage() {
  ...
  function exportPDF() {
    const html = `<html><head><title>Repair Job Sheet</title><style>body{font-family:sans-serif;font-size:11px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ccc;padding:5px 7px}th{background:#0d7070;color:#fff}</style></head><body><h2>Repair Job Sheet</h2><h3>${customer ? `${customer.first_name} ${customer.last_name ?? ''} -- ${customer.phone ?? ''}` : ''}</h3><table><thead><tr>${EXPORT_COLS.map(h => `<th>${h}</th>`).join('')}</tr></thead><tbody>${repairs.map(r => `<tr>${row2arr(r, businessName).map(v => `<td>${v}</td>`).join('')}</tr>`).join('')}</tbody></table></body></html>`
