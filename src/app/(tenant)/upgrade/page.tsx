...
export default function UpgradePage() {
  ...
  return (
    <div className="min-h-screen bg-surface-container-low flex flex-col items-center justify-start py-16 px-4">
      {/* Banner */}
      ...
      <h1 className="text-3xl font-black text-on-surface mb-2 text-center">Choose your plan</h1>
      <p className="text-on-surface-variant text-sm mb-10 text-center">No hidden fees. Cancel anytime.</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full max-w-6xl">
          {plans.map((plan, index) => {
            ...
            return (
              ...
              >
                {highlighted && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-amber-400 text-on-surface text-xs font-bold px-3 py-1 rounded-full shadow">
                    Most popular
                  </span>
                )}
                ...
                <div className="flex items-end gap-1 mb-2">
                  <span className={`text-5xl font-black leading-none ${highlighted ? 'text-white' : 'text-on-surface'}`}>
                    &pound;{plan.price_monthly}
                  </span>
                  <span className={`text-base mb-1 ${highlighted ? 'text-indigo-200' : 'text-outline'}`}>/mo</span>
                </div>
                <p className={`text-xs mb-6 ${highlighted ? 'text-indigo-200' : 'text-outline'}`}>Billed monthly, cancel anytime</p>

                {/* Stats */}
                <div className="flex gap-2 mb-6">
                  <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${highlighted ? 'bg-indigo-500 text-white' : 'bg-surface-container text-on-surface-variant'}`}>
                ...
                <ul className="flex-1 space-y-2 mb-8">
                  {(plan.features ?? []).map((f) => (
                    <li key={f} className="flex items-center gap-2 text-sm">
                      ...
                      <span className={`capitalize ${highlighted ? 'text-white' : 'text-on-surface-variant'}`}>{f.replace(/_/g, ' ')}</span>
                    </li>
                  ))}
                </ul>
                ...
                <Button
                  ...
                  className={`w-full font-bold py-3 rounded-xl ${
                    highlighted
                      ? 'bg-surface text-indigo-700 hover:bg-indigo-50'
