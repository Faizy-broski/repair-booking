  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-outline-variant bg-surface overflow-hidden">
        ...
        <div className="lg:hidden border-b border-outline-variant px-4 py-2 bg-surface-container-low flex items-center gap-1 flex-wrap">
          <button onClick={() => setMobileStep(0)} className={`text-xs px-2.5 py-1 rounded-full font-medium transition-colors ${mobileStep === 0 ? 'bg-brand-teal text-white' : 'text-on-surface-variant hover:text-on-surface-variant'}`}>Types</button>
          {selectedTypeId && (<>
            <ChevronRight className="h-3 w-3 text-outline-variant shrink-0" />
            <button onClick={() => setMobileStep(1)} className={`text-xs px-2.5 py-1 rounded-full font-medium transition-colors ${mobileStep === 1 ? 'bg-brand-teal text-white' : 'text-on-surface-variant hover:text-on-surface-variant'}`}>
              {categories.find(c => c.id === selectedTypeId)?.name ?? 'Brands'}
            </button>
          </>)}
          {selectedBrandId && (<>
            <ChevronRight className="h-3 w-3 text-outline-variant shrink-0" />
            <button onClick={() => setMobileStep(2)} className={`text-xs px-2.5 py-1 rounded-full font-medium transition-colors ${mobileStep === 2 ? 'bg-brand-teal text-white' : 'text-on-surface-variant hover:text-on-surface-variant'}`}>
              {brands.find(b => b.id === selectedBrandId)?.name ?? 'Models'}
            </button>
          </>)}
          {selectedModelId && (<>
            <ChevronRight className="h-3 w-3 text-outline-variant shrink-0" />
            <button onClick={() => setMobileStep(3)} className={`text-xs px-2.5 py-1 rounded-full font-medium transition-colors ${mobileStep === 3 ? 'bg-brand-teal text-white' : 'text-on-surface-variant hover:text-on-surface-variant'}`}>Parts</button>
          </>)}
        </div>

        {/* 4-column explorer */}
        <div className="lg:grid lg:grid-cols-4 lg:divide-x lg:divide-outline-variant" style={{ minHeight: 520 }}>
          ...
          <div className={`flex flex-col border-b lg:border-b-0 border-outline-variant ${mobileStep !== 0 ? 'hidden lg:flex' : 'flex'}`}>
            <div className="flex items-center justify-between px-4 py-2 bg-surface-container-low border-b border-outline-variant">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-outline">Device Types</span>
              <span className="text-xs text-outline bg-surface-container rounded-full px-1.5">{categories.length}</span>
            </div>
            <div className="px-3 py-2 border-b border-outline-variant">
              <div className="flex gap-2 items-center">
                ...
                <input type="text" placeholder="e.g. Phones, Laptops…" value={newTypeName}
                  ...
                  className="h-10 flex-1 min-w-0 rounded-md border border-outline-variant px-3 text-[15px] focus:border-brand-teal focus:outline-none" />
            ...
            <div className="flex-1 overflow-y-auto">
              {categories.length === 0 && <p className="text-xs text-outline text-center py-10">No device types yet</p>}
              {categories.map(dt => {
                ...
                return (
                  <div key={dt.id}
                    ...
                    className={`flex items-center gap-3 px-4 py-3.5 cursor-pointer border-b border-outline-variant last:border-b-0 transition-colors ${isSelected ? 'bg-teal-50 border-l-[3px] border-l-brand-teal' : 'hover:bg-surface-container-low border-l-[3px] border-l-transparent'}`}>
                    {dt.image_url
                      ? <img src={dt.image_url} alt={dt.name} className="h-8 w-8 rounded-lg object-cover border border-outline-variant shrink-0 bg-surface" />
                    ...
                    {isEditing ? (
                      ...
                      <>
                        <span className="flex-1 text-[15px] font-medium text-on-surface truncate">{dt.name}</span>
                        <span className="text-xs font-semibold text-outline shrink-0 mr-2">{brands.filter(b => b.category_id === dt.id).length}</span>
                        <div className="flex gap-1 shrink-0" onClick={e => e.stopPropagation()}>
                          <button onClick={() => { setEditingTypeId(dt.id); setEditTypeName(dt.name); setEditTypeImage(dt.image_url ?? '') }} className="p-1.5 rounded text-outline hover:text-on-surface-variant hover:bg-surface-container"><Pencil className="h-3.5 w-3.5" /></button>
                          <button onClick={() => deleteDeviceType(dt.id)} className="p-1.5 rounded text-outline hover:text-red-500 hover:bg-red-50"><Trash2 className="h-3.5 w-3.5" /></button>
                        </div>
                      </>
                    )}
                  </div>
          ...
          <div className={`flex flex-col border-b lg:border-b-0 border-outline-variant ${mobileStep !== 1 ? 'hidden lg:flex' : 'flex'}`}>
            <div className="flex items-center justify-between px-4 py-2 bg-surface-container-low border-b border-outline-variant">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-outline">Brands</span>
              {selectedTypeId && <span className="text-xs text-outline bg-surface-container rounded-full px-1.5">{brands.filter(b => b.category_id === selectedTypeId).length}</span>}
            </div>
            {!selectedTypeId ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-2 text-center px-6 py-10">
                <Tag className="h-9 w-9 text-outline-variant" />
                <p className="text-xs text-outline">Select a device type first</p>
              </div>
            ) : (
              <>
                <div className="px-3 py-2 border-b border-outline-variant">
                ...
                <div className="flex-1 overflow-y-auto">
                  {brands.filter(b => b.category_id === selectedTypeId).length === 0 && <p className="text-xs text-outline text-center py-10">No brands yet</p>}
                  {brands.filter(b => b.category_id === selectedTypeId).map(brand => {
                    ...
                    return (
                      <div key={brand.id}
                        ...
                        className={`flex items-center gap-3 px-4 py-3.5 cursor-pointer border-b border-outline-variant last:border-b-0 transition-colors ${isSelected ? 'bg-teal-50 border-l-[3px] border-l-brand-teal' : 'hover:bg-surface-container-low border-l-[3px] border-l-transparent'}`}>
                        {brand.image_url
                          ? <img src={brand.image_url} alt={brand.name} className="h-8 w-8 rounded-lg object-contain border border-outline-variant shrink-0 bg-surface" />
                        ...
                        {isEditing ? (
                          ...
                          <>
                            <span className="flex-1 text-[15px] font-medium text-on-surface truncate">{brand.name}</span>
                            <span className="text-xs font-semibold text-outline shrink-0 mr-2">{models.filter(m => m.brand_id === brand.id).length}</span>
                            <div className="flex gap-1 shrink-0" onClick={e => e.stopPropagation()}>
                              <button onClick={() => { setEditingBrandId(brand.id); setEditBrandName(brand.name); setEditBrandImage(brand.image_url ?? '') }} className="p-1.5 rounded text-outline hover:text-on-surface-variant hover:bg-surface-container"><Pencil className="h-3.5 w-3.5" /></button>
          ...
          <div className={`flex flex-col border-b lg:border-b-0 border-outline-variant ${mobileStep !== 2 ? 'hidden lg:flex' : 'flex'}`}>
            <div className="flex items-center justify-between px-4 py-2 bg-surface-container-low border-b border-outline-variant">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-outline">Models</span>
              {selectedBrandId && <span className="text-xs text-outline bg-surface-container rounded-full px-1.5">{models.filter(m => m.brand_id === selectedBrandId).length}</span>}
            </div>
            {!selectedBrandId ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-2 text-center px-6 py-10">
                <Package className="h-9 w-9 text-outline-variant" />
                <p className="text-xs text-outline">Select a brand first</p>
              </div>
            ) : (
              <>
                <div className="px-3 py-2 border-b border-outline-variant">
                ...
                <div className="flex-1 overflow-y-auto">
                  {models.filter(m => m.brand_id === selectedBrandId).length === 0 && <p className="text-xs text-outline text-center py-10">No models yet</p>}
                  {models.filter(m => m.brand_id === selectedBrandId).map(model => {
                    ...
                    return (
                      <div key={model.id}
                        ...
                        className={`flex items-center gap-3 px-4 py-3.5 cursor-pointer border-b border-outline-variant last:border-b-0 transition-colors ${isSelected ? 'bg-teal-50 border-l-[3px] border-l-brand-teal' : 'hover:bg-surface-container-low border-l-[3px] border-l-transparent'}`}>
                        {model.image_url
                          ? <img src={model.image_url} alt={model.name} className="h-8 w-8 rounded-lg object-cover border border-outline-variant shrink-0 bg-surface" />
                          : <div className="h-8 w-8 rounded-lg bg-surface-container flex items-center justify-center shrink-0 border border-outline-variant"><Package className="h-4 w-4 text-on-surface-variant" /></div>}
                        {isEditing ? (
                          <>
                            ...
                            <div className="flex gap-1 shrink-0" onClick={e => e.stopPropagation()}>
                              ...
                              <button onClick={() => setEditingModelId(null)} className="p-1 rounded text-outline hover:bg-surface-container"><X className="h-3.5 w-3.5" /></button>
                            </div>
                          </>
                        ) : (
                          <>
                            <span className="flex-1 text-[15px] font-medium text-on-surface truncate">{model.name}</span>
                            <span className="text-xs font-semibold text-outline shrink-0 mr-2">{partCount} parts</span>
                            <div className="flex gap-1 shrink-0" onClick={e => e.stopPropagation()}>
                              <button onClick={() => { setEditingModelId(model.id); setEditModelName(model.name); setEditModelImage(model.image_url ?? '') }} className="p-1.5 rounded text-outline hover:text-on-surface-variant hover:bg-surface-container"><Pencil className="h-3.5 w-3.5" /></button>
                              <button onClick={() => deleteModel(model.id)} className="p-1.5 rounded text-outline hover:text-red-500 hover:bg-red-50"><Trash2 className="h-3.5 w-3.5" /></button>
                            </div>
                          </>
                        )}
                      </div>
          ...
          <div className={`flex flex-col ${mobileStep !== 3 ? 'hidden lg:flex' : 'flex'}`}>
            <div className="flex items-center justify-between px-4 py-2 bg-surface-container-low border-b border-outline-variant">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[11px] font-semibold uppercase tracking-wider text-outline">Part Types</span>
              </div>
              {selectedModelId && <span className="text-xs text-outline bg-surface-container rounded-full px-1.5">{partTypes.filter(p => p.device_id === selectedModelId).length}</span>}
            </div>
            {!selectedModelId ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-2 text-center px-6 py-10">
                <Cpu className="h-9 w-9 text-outline-variant" />
                <p className="text-xs text-outline">Select a model first</p>
              </div>
            ) : (
              <>
                <div className="px-3 py-2 border-b border-outline-variant">
                  <div className="flex gap-2 items-center">
                    ...
                    <input type="text" placeholder="e.g. Screen, Battery, IC…" value={newPartName}
                      ...
                      className="h-10 flex-1 min-w-0 rounded-md border border-outline-variant px-3 text-[15px] focus:border-purple-400 focus:outline-none" />
                ...
                <div className="flex-1 overflow-y-auto">
                  {partTypes.filter(p => p.device_id === selectedModelId).length === 0 && <p className="text-xs text-outline text-center py-10">No part types yet</p>}
                  {partTypes.filter(p => p.device_id === selectedModelId).map(pt => {
                    const isEditing = editingPartId === pt.id
                    return (
                      <div key={pt.id} className="flex items-center gap-3 px-4 py-3.5 border-b border-outline-variant last:border-b-0 hover:bg-purple-50/40 transition-colors">
                        {pt.image_url
                          ? <img src={pt.image_url} alt={pt.name} className="h-8 w-8 rounded-lg object-cover border border-outline-variant shrink-0 bg-surface" />
                        ...
                        {isEditing ? (
                          <>
                            ...
                            <div className="flex gap-1 shrink-0">
                              ...
                              <button onClick={() => setEditingPartId(null)} className="p-1 rounded text-outline hover:bg-surface-container"><X className="h-3.5 w-3.5" /></button>
                            </div>
                          </>
                        ) : (
                          <>
                            <span className="flex-1 text-[15px] font-medium text-on-surface truncate">{pt.name}</span>
                            <div className="flex gap-1 shrink-0">
                              <button onClick={() => { setEditingPartId(pt.id); setEditPartName(pt.name); setEditPartImage(pt.image_url ?? '') }} className="p-1.5 rounded text-outline hover:text-purple-600 hover:bg-purple-50"><Pencil className="h-3.5 w-3.5" /></button>
                              <button onClick={() => deletePartType(pt.id)} className="p-1.5 rounded text-outline hover:text-red-500 hover:bg-red-50"><Trash2 className="h-3.5 w-3.5" /></button>
                            </div>
                          </>
                        )}
                      </div>
                ...
                <div className="px-4 py-2 border-t border-outline-variant bg-surface-container-low/60">
                  <p className="text-[11px] text-outline truncate">
                    <span className="font-medium text-on-surface-variant">{models.find(m => m.id === selectedModelId)?.name}</span>
