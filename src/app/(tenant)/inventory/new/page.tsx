...
export default function NewInventoryPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <div className="sticky top-0 z-30 flex items-center justify-between border-b border-outline-variant bg-surface px-6 py-3">
        <div className="flex items-center gap-3">
          <Link href="/inventory" className="flex items-center gap-1.5 text-sm text-on-surface-variant hover:text-on-surface">
            <ChevronLeft className="h-4 w-4" /> Back to Inventory
          </Link>
          <span className="text-outline-variant">/</span>
          <span className="text-sm font-medium text-on-surface">Add New {itemType === 'part' ? 'Part' : 'Product'}</span>
        </div>
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-6xl px-6 py-6 space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-on-surface-variant mb-1">{hasRepairs ? 'Device Type' : 'Category'} <span className="text-red-500">*</span> <span className="text-xs font-normal text-outline">(select or create)</span></label>
                ...
                <div>
                  <label className={`block text-sm font-medium mb-1 ${!categoryId ? 'text-outline' : 'text-on-surface-variant'}`}>Brand <span className="text-red-500">*</span> <span className="text-xs font-normal text-outline">(select or create)</span></label>
                  {categoryId ? (
              ...
              {hasRepairs && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className={`block text-sm font-medium mb-1 ${!brandId ? 'text-outline' : 'text-on-surface-variant'}`}>Model <span className="text-xs font-normal text-outline">(select or create)</span></label>
                    {brandId ? (
                  ...
                  {itemType === 'part' && (
                    <div>
                      <label className={`block text-sm font-medium mb-1 ${!modelId ? 'text-outline' : 'text-on-surface-variant'}`}>Part Type <span className="text-red-500">*</span> <span className="text-xs font-normal text-outline">(select or create)</span></label>
                      {modelId ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-on-surface-variant mb-1">Barcode / UPC</label>
                  <div className="flex gap-2">
                    <input type="text" placeholder="Optional" value={barcode} onChange={e => setBarcode(e.target.value)} className={`flex-1 rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-teal/30 focus:border-brand-teal ${barcodeConflict ? 'border-red-400 bg-red-50' : 'border-outline'}`} />
            ...
            <div className="lg:col-span-1 space-y-6 lg:sticky lg:top-[76px]">
              ...
              <SectionCard title="Stock">
                ...
                {hasVariants && (
                  <p className="text-xs text-on-surface-variant rounded-lg border border-dashed border-outline-variant px-3 py-2">Opening stock is set per-variant in the table above.</p>
                )}
                <div>
                ...
                <div>
                  <label className="block text-sm font-medium text-on-surface-variant mb-1">Supplier <span className="text-xs font-normal text-outline">(select or create)</span></label>
                ...
                <div className="rounded-lg border border-outline-variant bg-surface-container-low p-4 space-y-3">
                  ...
                  {(() => {
                    ...
                    return attrsToShow.length > 0 ? (
                      ...
                      <p className="text-xs text-outline italic">No attributes linked to this category yet. <a href="/inventory/attributes" className="text-brand-teal underline">Add some in Attributes.</a></p>
                    ) : null
                  })()}
                  {attrDefs.map((attr, idx) => (
                    <div key={attr.id} className="flex items-center gap-2">
                      <div className="w-36 shrink-0">
                        <input type="text" placeholder={`Attribute ${idx + 1}`} value={attr.name} onChange={e => updateAttr(attr.id, 'name', e.target.value)} className="w-full rounded-lg border border-outline-variant bg-surface px-3 py-1.5 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                      </div>
                      <div className="flex-1">
                        <input type="text" placeholder="Values, comma-separated (e.g. Black, White, Silver)" value={attr.valuesRaw} onChange={e => updateAttr(attr.id, 'valuesRaw', e.target.value)} className="w-full rounded-lg border border-outline-variant bg-surface px-3 py-1.5 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                      </div>
                  ...
                  <div>
                    <div className="mb-2 flex items-center justify-between">
                      <p className="text-sm font-medium text-on-surface-variant flex items-center gap-1.5">
                        <Layers className="h-4 w-4 text-outline" /> {variantRows.length} variant{variantRows.length !== 1 ? 's' : ''}
                      </p>
                    ...
                    <div className="rounded-lg border border-outline-variant overflow-x-auto">
                      <table className="w-full divide-y divide-outline-variant text-sm">
                        <thead className="bg-surface-container-low">
                          <tr>
                            <th className="px-4 py-3 text-left text-sm font-semibold text-on-surface-variant">Image</th>
                            <th className="px-4 py-3 text-left text-sm font-semibold text-on-surface-variant">Variant</th>
                            <th className="px-4 py-3 text-left text-sm font-semibold text-on-surface-variant">SKU</th>
                            <th className="px-4 py-3 text-left text-sm font-semibold text-on-surface-variant">Barcode</th>
                            <th className="px-4 py-3 text-left text-sm font-semibold text-on-surface-variant">Cost ({currSymbol})</th>
                            <th className="px-4 py-3 text-left text-sm font-semibold text-on-surface-variant">Price ({currSymbol}) *</th>
                            <th className="px-4 py-3 text-left text-sm font-semibold text-on-surface-variant">Stock</th>
                            <th className="px-4 py-3" />
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-outline-variant bg-surface">
                          {variantRows.map(row => (
                            <tr key={row.key} className="hover:bg-surface-container-low/60 align-middle">
                              <td className="px-4 py-3">
                                {row.imageUrl ? (
                                  <div className="relative w-12 h-12 group">
                                    <img src={row.imageUrl} alt={row.name} className="w-12 h-12 rounded-lg object-cover border border-outline-variant" />
                                  ...
                                  <label className="flex w-12 h-12 cursor-pointer items-center justify-center rounded-lg border border-dashed border-outline bg-surface-container-low hover:border-brand-teal hover:bg-brand-teal/5 transition-colors">
                                    <Plus className="h-5 w-5 text-outline" />
                              ...
                              <td className="px-4 py-3 font-semibold text-sm text-on-surface whitespace-nowrap">{row.name}</td>
                              <td className="px-4 py-3"><input type="text" value={row.sku} onChange={e => updateVariantRow(row.key, 'sku', e.target.value)} placeholder="Optional" className="w-24 rounded-lg border border-outline-variant px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-teal/30 focus:border-brand-teal" /></td>
                              <td className="px-4 py-3">
                                <div className="flex items-center gap-1.5">
                                  <input type="text" value={row.barcode} onChange={e => updateVariantRow(row.key, 'barcode', e.target.value)} placeholder="Optional" className="w-28 rounded-lg border border-outline-variant px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-teal/30 focus:border-brand-teal" />
                              ...
                              <td className="px-4 py-3"><input type="number" min="0" step="0.01" value={row.costPrice} onChange={e => updateVariantRow(row.key, 'costPrice', e.target.value)} placeholder="0.00" className="w-24 rounded-lg border border-outline-variant px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-teal/30 focus:border-brand-teal" /></td>
                              <td className="px-4 py-3"><input type="number" min="0" step="0.01" value={row.sellingPrice} onChange={e => updateVariantRow(row.key, 'sellingPrice', e.target.value)} placeholder="0.00" className={`w-24 rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-teal/30 focus:border-brand-teal ${!row.sellingPrice ? 'border-red-300 bg-red-50' : 'border-outline-variant'}`} /></td>
                              <td className="px-4 py-3"><input type="number" min="0" value={row.stock} onChange={e => updateVariantRow(row.key, 'stock', e.target.value)} className="w-20 rounded-lg border border-outline-variant px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-teal/30 focus:border-brand-teal" /></td>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <SectionCard
                ...
                description={hasVariants
                  ? 'Pre-filled into all generated variants — override each one above.'
          ...
          <div className="flex flex-col sm:flex-row items-center justify-end gap-3 py-6 border-t border-outline-variant">
